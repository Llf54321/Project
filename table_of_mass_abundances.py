'''
    Create the table of Isotopic Masses and Natural Abundances
'''
...
def add_mass_abundance(page, da):
    ...
    for i in range(3,len(tables[0].df[2])):
        ...
        if x != []:
            ...
            if tables[0].df[4].values[i] != '*':
                ...
                if form_1 not in da.keys():
                    da[form_1] = [(mass_1,abundance_1)]
                else:
                    da[form_1].append((mass_1,abundance_1))
            else:
                if form_1 not in da.keys():
                    da[form_1] = [(mass_1,0)]
                else:
                    da[form_1].append((mass_1,0))
        if y != []:
            mass_2 = int(y[0])
            form_2 = y[1]
            if tables[0].df[9].values[i] != '*':
                abundance_2 = float(tables[0].df[9].values[i])
                if form_2 not in da.keys():
                    da[form_2] = [(mass_2,abundance_2)]
                else:
                    da[form_2].append((mass_2,abundance_2))
            else:
                if form_2 not in da.keys():
                    da[form_2] = [(mass_2,0)]
                else:
                    da[form_2].append((mass_2,0))
